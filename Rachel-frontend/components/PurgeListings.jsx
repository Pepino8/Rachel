...
function PurgeListings() {
    ...
    const handlePurgeExpired = async () => {
        ...
        console.log("Requesting Gameflip expired and draft listings purge...");
        try {
            const response = await axios.post(`${API_URL}/api/purge/expired`);
            console.log("Purge expired response:", response.data);
            const count = response.data?.purged ?? 0;
            setLastPurgeResult({ count, type: 'expired', timestamp: new Date() });

            if (count > 0) {
                showToast(`Purged ${count} expired/draft listings successfully!`, 'success');
            } else {
                showToast('No expired or draft listings found. Your inventory is up to date!', 'info');
            }

            window.dispatchEvent(new CustomEvent('listings_purged', { detail: { count, type: 'expired' } }));
        } catch (error) {
            console.error("Purge expired failed:", error.response?.data || error.message);
            showToast(`Purge failed: ${extractErrorMessage(error)}`, 'error');
        } finally {
    ...
    const executePurgeAll = async () => {
        ...
        try {
            ...
            console.log("Purge all response:", response.data);
            const count = response.data?.purged ?? 0;
            setLastPurgeResult({ count, type: 'all', timestamp: new Date() });

            if (count > 0) {
                showToast(`Purged all ${count} listings from Gameflip!`, 'success');
            } else {
                showToast('No active or draft listings found to remove.', 'info');
            }

            window.dispatchEvent(new CustomEvent('listings_purged', { detail: { count, type: 'all' } }));
        } catch (error) {
            console.error("Purge all failed:", error.response?.data || error.message);
            showToast(`Purge all failed: ${extractErrorMessage(error)}`, 'error');
        } finally {
    ...
    return (
        <div className="bg-zinc-900/40 border border-zinc-800/80 backdrop-blur-md rounded-2xl p-6 shadow-xl shadow-black/20 w-full transition-all duration-300">
            ...
            <p className="text-sm text-zinc-400 leading-relaxed mb-6">
                Clean up active listing records. Any listing published 24 hours or more ago is automatically removed by the background agent. You can also trigger an immediate manual purge below.
            </p>

            {/* Stats / Feedback dashboard */}
            {lastPurgeResult && (
                <div className="grid grid-cols-1 gap-4 mb-6 animate-in fade-in duration-200">
                    {lastPurgeResult.count > 0 ? (
                        <div className="bg-zinc-950/40 border border-zinc-800/50 rounded-xl p-4 flex items-center justify-between transition-all duration-300 hover:border-zinc-700/50">
                            <div>
                                <span className="text-xs font-semibold uppercase tracking-wider text-zinc-500">
                                    {lastPurgeResult.type === 'expired' ? 'Expired / Drafts Removed' : 'All Listings Cleared'}
                                </span>
                                <div className="text-3xl font-extrabold text-red-400 tracking-tight mt-1">
                                    {lastPurgeResult.count}
                                </div>
                            </div>
                        ...
                        <div className="bg-zinc-950/40 border border-zinc-800/50 rounded-xl p-4 flex items-center justify-between transition-all duration-300 hover:border-zinc-700/50">
                            <div>
                                ...
                                <div className="text-base font-bold text-emerald-400 tracking-tight mt-1 flex items-center gap-1.5">
                                    Inventory is Up to Date (0 found)
                                </div>
                            </div>
                ...
                <button
                    ...
                    className={`w-full py-2.5 px-4 rounded-xl font-semibold text-sm transition-all duration-200 cursor-pointer flex items-center justify-center gap-2 border disabled:opacity-50 disabled:cursor-not-allowed
                        ${isPurgingExpired
                            ...
                            : 'bg-zinc-800/80 hover:bg-zinc-700 text-zinc-200 border-zinc-700/50 hover:border-zinc-600/50 active:scale-[0.99]'
                        }`}
                >
                    {isPurgingExpired ? (
                        <>
                            ...
                            <span>Purging Expired & Drafts...</span>
                        </>
                    ) : (
                        <>
                            ...
                            <span>Remove Expired / Drafts</span>
                        </>
                    )}
                </button>
                ...
                <button
                    ...
                    className={`w-full py-2.5 px-4 rounded-xl font-semibold text-sm transition-all duration-200 cursor-pointer flex items-center justify-center gap-2 border disabled:opacity-50 disabled:cursor-not-allowed
                        ${isPurgingAll
                            ...
                            : 'bg-red-950/10 hover:bg-red-950/30 text-red-400 border-red-950/30 hover:border-red-500/30 active:scale-[0.99]'
                        }`}
                >
                    {isPurgingAll ? (
                        <>
                            ...
                            <span>Purging All Listings...</span>
                        </>
                    ) : (
                        <>
